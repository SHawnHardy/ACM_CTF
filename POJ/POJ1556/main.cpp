...
typedef pair<pt,double> ppd;
vector<ppd> p[maxn];
vector<ln> w[maxn];

double dp(pt a,int xn) {
    double re=dist(p[xn-1][0].first,a)+p[xn-1][0].second;
    for (int i=0; i<xn; i++) {
        for (int j=0; j<p[i].size(); j++) {
            ...
            if (flag)
                re=min(re,dist(p[i][j].first,a)+p[i][j].second);
        }
    }
    return re;
}
...
void sol() {
    ...
    p[0].push_back(make_pair(pt(0.0,5.0),0.0));
    ...
    for(int i=1; i<=n; i++) {
        ...
        double ty;
        scanf("%lf",&ty);
        p[i].push_back(make_pair(pt(x,ty), dp(pt(x,ty),i)));
        w[i].push_back(ln(pt(x,0),pt(x,ty)));
        double ty1,ty2;
        scanf("%lf%lf",&ty1,&ty2);
        p[i].push_back(make_pair(pt(x,ty1), dp(pt(x,ty1),i)));
        p[i].push_back(make_pair(pt(x,ty2), dp(pt(x,ty2),i)));
        
        w[i].push_back(ln(pt(x,ty1),pt(x,ty2)));
        
        double ty3;
        scanf("%lf",&ty3);
        p[i].push_back(make_pair(pt(x,ty3), dp(pt(x,ty3),i)));
        w[i].push_back(ln(pt(x,ty3),pt(x,10.0)));
    }
    printf("%.2f\n",dp(pt(10.0,5.0), n+1));
}
