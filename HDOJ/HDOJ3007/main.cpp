...
double smallestCc(vector<pt> &P,pt &c) {
    random_shuffle(P.begin(),P.end());
    c=P[0];
    double re=0;
    for (int i=1; i<(int)P.size(); i++) {
        if (sgn(dist(P[i],c)-re)==1) {
            c=P[i];
            re=0;
            for (int j=0; j<i; j++) {
                if (sgn(dist(P[j],c)-re)==1) {
                    c=(P[i]+P[j])/2.0;
                    re=dist(c,P[j]);
                    for (int k=0; k<j; k++) {
                        if (sgn(dist(P[k],c)-re)==1) {
                            c=triCircumCenter(P[i], P[j], P[k]);
                            re=dist(c,P[i]);
                        }
                    }
                }
